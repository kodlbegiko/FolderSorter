...
The positioning should stay aligned with [PRODUCT_STRATEGY.md](PRODUCT_STRATEGY.md):
FolderSorter is not trying to be the strongest automation tool; it should be the
file organizer everyday users feel safe pressing Apply on.
...
- Keep product positioning visible in `PRODUCT_STRATEGY.md` and `PRODUCT_STRATEGY.zh-TW.md`.
...
GitHub repository search uses the repository name, description, and topics by
default. README content is also searchable through the `in:readme` qualifier.
Keep the most important user-intent terms in the first three surfaces instead of
repeating keywords unnaturally throughout the repository.
...
- `SwiftUI file organizer`
- `Python file organizer CLI`

Canonical repository description:

```text
Open-source, local-first macOS file organizer and Downloads cleaner with preview, undo, SwiftUI GUI, and Python CLI.
```

Metadata maintenance rules:

- Keep the H1 focused on `FolderSorter` and `macOS File Organizer`.
- Keep the first paragraph readable while covering preview, undo, local-first, SwiftUI, and Python CLI.
- Use repository topics for platform, use case, and implementation keywords.
- Link the GitHub Release and PyPI package near the top of the README.
- Keep the real product screenshot above the installation instructions.
- Upload `Media/foldersorter-social-preview.jpg` in GitHub Settings so shared links use the product UI instead of GitHub's generated card.
- Re-check repository search after metadata changes; GitHub indexing can take time.
